...
layout(binding = 2, rgba8) uniform image2D previousFrameImage;
...
void main() {
    ...
    // 1. Gyro Stabilization Offset
    ivec2 shiftedCoord = pixelCoord + ivec2(pc.gyroShiftX, pc.gyroShiftY);
    shiftedCoord = clamp(shiftedCoord, ivec2(0), ivec2(pc.width - 1, pc.height - 1));

    vec4 rawPx = imageLoad(inputImage, shiftedCoord);
    vec3 rgb = rawPx.rgb;

    // Temporal Optical Flow
    vec4 prevPx = imageLoad(previousFrameImage, pixelCoord);
    vec2 motionVector = (vec2(pixelCoord) - vec2(pc.gyroShiftX, pc.gyroShiftY)) - vec2(pixelCoord);
    rgb = mix(rgb, prevPx.rgb, clamp(length(motionVector) * 0.05f, 0.0f, 0.2f));

    vec3 lumaWeights = vec3(0.2126f, 0.7152f, 0.0722f);
    float luma = dot(rgb, lumaWeights);

    // 2. Photon Boost & Dark-Matter Recovery
    if (luma < 0.20f) {
        float photonBoost = 1.0f + ((0.20f - luma) * 7.5f);
        rgb = min(vec3(1.0f), rgb * photonBoost);
    }

    // 3. Zero-Noise Wavelet & Subgroup Denoise
    ...
    mat4 vMat = mat4(
        pc.viewMatrix[0], pc.viewMatrix[1], pc.viewMatrix[2], pc.viewMatrix[3],
        pc.viewMatrix[4], pc.viewMatrix[5], pc.viewMatrix[6], pc.viewMatrix[7],
        pc.viewMatrix[8], pc.viewMatrix[9], pc.viewMatrix[10], pc.viewMatrix[11],
        pc.viewMatrix[12], pc.viewMatrix[13], pc.viewMatrix[14], pc.viewMatrix[15]
    );
    vec4 ndcPos = vMat * vec4(float(pixelCoord.x) / float(pc.width), float(pixelCoord.y) / float(pc.height), luma, 1.0f);
    float depthFactor = clamp(ndcPos.z * 0.1f, 0.9f, 1.1f);
    rgb *= depthFactor;

    // 7. ACES Filmic Tone Mapping Curve
    vec3 r_num = rgb * (2.51f * rgb + 0.03f);
    vec3 r_den = rgb * (2.43f * rgb + 0.59f) + 0.14f;
    rgb = clamp(r_num / r_den, 0.0f, 1.0f);

    // 8. Hollywood Color Grade & Depth Mask
    rgb.r *= 1.15f;
    rgb.b *= 0.88f;
    ...
    imageStore(outputImage, pixelCoord, vec4(rgb, rawPx.a));
}
...
Java_com_my_newproject_truesingularityclass_nativeProcessAiEnhancement(JNIEnv *env, jclass clazz, jobject targetBitmap, jstring outputPath) {
    ...
    ncnn::Mat inMat = ncnn::Mat::from_android_bitmap_resize(pixels, info.width, info.height, ncnn::Mat::PIXEL_RGBA2RGB);
    ncnn::Mat outMat;
